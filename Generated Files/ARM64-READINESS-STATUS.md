...
- Second follow-up fix in this pass (OPENSHOT-TEST-CLEANUP-001):
  - **Observed defect:** the `build\install-arm64` regression coverage in `test_teardown_preserves_unrelated_preexisting_build_msix_output` wrote its sentinel to the production path `build\install-arm64\bin\openshot-qt.exe` (risking overwrite of a genuine installed binary) and its `finally` block then called `remove_path(INSTALL_ARM64_DIR)`, recursively deleting the entire `build\install-arm64` tree, including any unrelated real install payload alongside the sentinel.
  - **Fix applied:** the test now seeds two test-unique, non-production filenames under `build\install-arm64\bin` and `build\install-arm64\lib` (never `openshot-qt.exe`) and its `finally` block removes only those exact owned files, then `rmdir`s their parent directories and `INSTALL_ARM64_DIR` itself only if empty, matching the same owned-paths discipline already used for `build\msix`.
  - **Regression coverage strengthened:** the existing regression test now also asserts that an unrelated pre-existing file under `build\install-arm64\lib` survives the full test including its `finally` cleanup, in addition to the pre-existing `build\install-arm64\bin` sentinel. Seeded reproduction (`build\install-arm64\lib\real-preexisting-payload.dll` and `build\install-arm64\bin\openshot-qt.exe` created before running the full suite) confirmed both survive after `python -m unittest discover -s ci -p test_package_msix_staging.py -v`.
